...
def load(fea_types, fea_path, dataset_file, vocabfile='', vocab={}, 
    ...
    for dialog in dialog_data['dialogs']:
        ...
        for n in six.moves.range(len(questions)):
            ...
            dialog_list.append((vid, qa_id, history, question, answer_in, answer_out, summary_in, summary_out))
            qa_id += 1
...
def make_batch(data, index, eos=1):
    ...
    for i in six.moves.range(n_seqs):
        qa_id = index[1][i]
        history, question, answer_in, answer_out, summary_in, summary_out = dialogs[qa_id][2:]
    ...
    return x_batch, h_batch, q_batch, a_batch_in, a_batch_out, s_batch, summary_batch_in, summary_batch_out
