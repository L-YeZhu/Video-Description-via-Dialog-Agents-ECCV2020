...
class HLSTMEncoder(nn.Module):
    ...
    def __call__(self, s, xs, **kwargs):
        ...
        sx = []
        # print("length of xs:",len(xs)) # num of qa pairs before the current question 
        for l in six.moves.range(len(xs)):
            if len(xs[l]) != 0: # if there is dialog history
                sections = np.array([len(x) for x in xs[l]], dtype=np.int32)
                #sections is an array(64 numbers) that contains the length of each sequence 
                #print("xs[l]:", xs[l])
        ...
        #print("sx size:", len(sx)) #sx is a list of length n, num of previous qa pairs

        # sentence level
        sxs = torch.stack(sx, dim=0)
        sxs = sxs.permute(1,0,2)
        # print("sxs size:", sxs.size())
