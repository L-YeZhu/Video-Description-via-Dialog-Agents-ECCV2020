...
from ee_mmseq2seq_model import MMSeq2SeqModel
from multimodal_encoder import MMEncoder
from lstm_encoder import LSTMEncoder
from hlstm_encoder import HLSTMEncoder
from summary_decoder import HLSTMDecoder
...
def evaluate(model, data, indices):
    ...
    with torch.no_grad():
        ...
        for j in six.moves.range(len(indices)):
            # get a fetched batch
            x_batch, h_batch, q_batch, a_batch_in, a_batch_out, s_batch, summary_batch_in, summary_batch_out = batch.pop()
            ...
            _, _, loss = model.loss(x, h, q, smi, smo)
